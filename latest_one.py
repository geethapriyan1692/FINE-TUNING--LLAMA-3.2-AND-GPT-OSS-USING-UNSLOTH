...
CONFIG = {
    # IO
    "RAW_DOCS_DIR": r"E:\Dataset_prep_pipeline\Documents",
    "OUT_DIR": "./dataset_out",
    ...
    "TEMPERATURE_QA": 0.20,      # slightly lower to improve format adherence
...
Output format (repeat as many pairs as found):
Q: <specific question ("How to", "How do I", "Where is", "List...", etc.)>
A:
1. <first step>
2. <second step>
- <bullets ONLY for enumerations>
...
def try_repair_placeholders(answer: str, context: str) -> str or None: # type: ignore
...
STEP_CAND_RE = re.compile(
    r'(?m)^\s*\d+\.\s+.+$|(?m)^\s*(Click|Select|Enter|Choose|Go to|Open|Press)\b.+',
    re.I
)
